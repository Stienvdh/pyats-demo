    ...
    def setup(self, testbed):
        ...
        for device_name, device in testbed.devices.items():
            ...
            if device.os in ("iosxe"):
                ...
                routes = self.routes[device_name]['routeTable']['vrf']['default']['address_family']['ipv4']['routes']
                
                if "0.0.0.0/0" in routes:
                    logger.info("Default gateway is: " + routes["0.0.0.0/0"]['next_hop']['next_hop_list'][1]['next_hop'])
                    self.passed()
                else: 
                    logger.info("No default gateway is set")
                    self.failed()
